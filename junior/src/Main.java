public class Main {
    ...
    private static String convertToIntervals(Integer[] input) {
        ...
        for (int i = 0; i < input.length; i++) {       //перебор всех элементов
            if (i == input.length - 2) {       //если это 2 последних элемента - записываем их и заканчиваем цикл(нет смысла проверять по условию)
                resultString.append(",").append(input[i]).append(",").append(input[i + 1]);
                break;
            } else if (i == input.length - 1) {        //если это 1 послдний элемент - ситуация как и с 2
                resultString.append(",").append(input[i]);
                break;
            } else if (input[i] + 2 == input[i + 2]) {        //если элемент через 2 позиции = значение + 2 данного элемента - подходит по условию
                ...
                int second = input[i + 1];     //создаем 2 число после тире и присваиваем значение следующего элемента
                for (int j = i; j < input.length; j++) {       //перебор элементов с позиции где нашелся нужный диапозон
                    if (input[j] - second == 1) {      //если элемент - следующий элемент == 1
                        second = input[j];     //записываем этот элемент
            ...
            } else if (input[i] + 2 != input[i + 2]) {        //если (значение элемента + 2) != (элемент + 2 позиции), то нет нужного диапозона(по условию)
                if (i == 0) {       //если это первая позиция - записываем без запятой
                    resultString.append(input[i]);
                    continue;
                }
                resultString.append(",").append(input[i]);     //записываем элемент с запятой
            }
        }
